"""
generate_lora.py: Generate text using LoRA fine-tuned GPT-2

This script loads a base GPT-2 model, applies LoRA adapters, and generates text.
...
Usage:
    ...
    # With different LoRA rank/alpha (must match training)
    python -m scripts.inference.generate_lora \
      --lora lora_checkpoints/lora_final.pt \
      --rank 16 \
      --alpha 32 \
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate text with LoRA fine-tuned GPT-2")
    ...
    parser.add_argument("--lora", type=str, required=True,
                        help="Path to LoRA weights checkpoint (REQUIRED)")
...
def generate(
    ...
    """
    Generate text from a LoRA-adapted model.

    Args:
        model: GPT model with LoRA adapters applied
...
def main():
    ...
    # Apply LoRA adapters
    # ========================================================================
    print(f"\nApplying LoRA structure (rank={args.rank}, alpha={args.alpha})...")
